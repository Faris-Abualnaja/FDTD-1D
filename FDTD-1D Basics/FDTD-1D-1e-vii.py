...
# Changed source to a wave packet (modulated Gaussian)
# Faris Abualnaja
# 2024-08-03
...
dy = 0.001       # Step size/cell size in space (y-direction)
...
def Source_Function(t):
    ...
    return np.exp(-0.5 * ((t_0 - t) / spread) ** 2)*np.sin(w_0*t*dt)
...
with writer.saving(fig, 'Gifs/FDTD-1D-1e-vii.gif', 100):
    # Time loop
    for n in range(n_max):
        ...
        if n % 5 == 0: # Frame rate
            ...
            plt.xticks(np.arange(0, 401, step=20))
            plt.xlim(0, 400)
